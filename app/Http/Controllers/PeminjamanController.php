...
class PeminjamanController extends Controller
{
  ...
  public function index(){
    if(Auth::user()->level=="petugas"){
      ...
      foreach ($peminjaman as $dt_pj){
        $ok=Detail_model::where('id_pinjam',$dt_pj->id)->get();
        $arr_detail=array();
        foreach ($ok as $ok) {
          $arr_detail[]=array(
  ...
  public function tampil(){
    ...
    ->join('anggota','anggota.id','=','peminjaman.id_anggota')
    ->join('petugas','petugas.id','=','peminjaman.id_petugas')
    ->select('peminjaman.id','anggota.nama_anggota','anggota.alamat','anggota.telp','petugas.nama_petugas','peminjaman.tgl_pinjam',
    'peminjaman.deadline','peminjaman.denda')
    ->get();
    $count=$data->count();
    $status=1;
